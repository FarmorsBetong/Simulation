...
public class SimView implements Observer {
    ...
    public void printResult(){
        ...
        System.out.println("1) av "+currentState.getTotalOfCustormers()+" kunder handlade "
                +String.valueOf(currentState.getTotalOfCustormers()-currentState.getMissed())+
                " medan" + currentState.getMissed()+" missades.");
        System.out.println("2) Totalt tid " + currentState.getAmOfRegs()+" kassor har varit"+
                " lediga: "+currentState.getFreeTimeRegs()+" te.");
        double snittTid = currentState.getFreeTimeRegs()/currentState.getAmOfRegs();
        double percentTid = snittTid/currentState.getTime();
        System.out.println("Genomsnittlig ledig kassatid: "+snittTid+
                "(dvs "+percentTid+"% av tiden från öppning till sista kunden betalat).");

        int totAmQueuedPeeps = currentState.getPeopleInLineTotal();
        double snittKöTid = currentState.getInLineTime()/totAmQueuedPeeps;
        System.out.println("3) Total tid "+totAmQueuedPeeps+" kunder"
                + " tvingats köa: "+currentState.getInLineTime()+" te.");
    ...
    public void update(Observable arg0, Object f) {
        String time = String.valueOf(currentState.getTime());
        String event = String.valueOf(currentState.getEventName());
        String custNum;
        ...
        String sumTimeFreeRegs = String.valueOf(currentState.getFreeTimeRegs());
